...
export const useGameDuelState = (
    ...
    const getWinnerOnTimeout = useCallback(
        (challenger: Player, defender: Player) => {
    ...
    const finishDuel = useCallback(
        (winningPlayer: Player, losingPlayer: Player, inheritedCategory: string) => {
    ...
    const tryAdvanceQuestionId = useCallback(
        async (currentId: number) => {
            ...
            if (exists) {
                ...
                notifyWarning(
                    `Koniec pytań w kategorii ${currentCategory}`,
                    `Brak pliku ${nextId}.${gameConfig.imageFileFormat}. Koniec pojedynku.`
                );
    ...
    useEffect(() => {
        ...
        conquerTerritory,
        setWinner,
        tryAdvanceQuestionId,
        questionId,
        finishDuel,
        gameConfig.passPenaltySeconds,
    ]);
    ...
    const addTimeBoostsToPlayerTimer = useCallback(
    ...
    const prepareDuelState = useCallback(
        (challengerPlayer: Player, defenderPlayer: Player) => {
