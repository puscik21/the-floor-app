...
export const useGameMapState = (
    ...
    const conquerTerritory = useCallback(
        (winnerName: string, loserName: string, inheritedCategory: string) => {
        ...
        [positionToPlayer, gameConfig.winStreakForTimeBoost]
    );
    ...
    const decreaseTimeBoostsOfPlayer = useCallback(
        (playerName: string) => {
            const newPlayers = allPlayers.map((p) =>
                p.name === playerName ? { ...p, timeBoostsAvailable: p.timeBoostsAvailable - 1 } : p
            );
