...
const FloorScreen = () => {
    const {
        map: { hasWonPreviousDuel, activeMapPlayer },
        actions: { handlePassFloorClick },
    } = useGameContext();
    ...
    return (
        <MapScreenWrapper>
            ...
            <PlayerGrid />
            <PassButton disabled={!hasWonPreviousDuel} onClick={handlePassFloorClick}>
                Pas
            </PassButton>
        </MapScreenWrapper>
    );
...
const FloorTitle = styled(Typography)`
    ...
    text-shadow: 0 6px 12px rgba(0, 40, 120, 0.6), 0 0 18px rgba(0, 140, 255, 0.25);
`;
